...
public class SignIn extends AppCompatActivity {
    ...
    private void divertUser(final String uid) {
        ...
        mDatabase1 = FirebaseDatabase.getInstance().getReference().child("USERS").child("Hospital Manager");
        mDatabase2 = FirebaseDatabase.getInstance().getReference().child("USERS").child("DRIVER_USER");

        mDatabase.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.hasChild(uid)){
                    Intent intent = new Intent(SignIn.this, genHome.class);
        ...
        mDatabase1.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.hasChild(uid)){
                    Intent intent = new Intent(SignIn.this, mngHome.class);
                    startActivity(intent);
                    overridePendingTransition(R.anim.right_to_left, R.anim.right_to_left_exit);
                    finish();
                }
